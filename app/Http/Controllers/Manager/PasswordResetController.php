...
class PasswordResetController extends Controller {
    ...
    public function sendResetLink(Request $request) {
        ...
        $message = 'Use This Link to Reset Password: ' . url('/') . '/manager/password/reset/' . $token;
    ...
    public function resetLink($token) {
        ...
        if (empty($reset) || $reset->status == 1) {
            ...
            return view('manager.passwords.reset', compact('reset'));
        }
    }
    ...
    public function passwordReset(Request $request) {
        ...
        if (empty($reset) || empty($user) || $reset->status == 1) {
            ...
            return redirect()->route('login')->withSuccess('success', 'Password Changed');
        }
    }
